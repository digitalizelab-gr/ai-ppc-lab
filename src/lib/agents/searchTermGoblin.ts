...
export function preprocessSearchTerms(dataset: StoredDataset): SearchTermEvidence {
  return buildSearchTermEvidence(dataset.rows as unknown as SearchTermRow[]);
}
...
- For every insight or action that makes a specific claim, add a matching short string to the "evidence" array citing the exact term/metric/value it rests on (e.g. "wasted $412.30 on \\"free resume templates\\" with 0 conversions across 340 clicks").
